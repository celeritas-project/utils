                        ...
                        IdToTrackMap& trackid_track_map)
{
    ...
    for (auto& key : trackid_track_map)
    {
        ...
        // Fill energy loss info
        for (int i = 1; i < key.second.steps.size(); i++)
        {
            auto const& prev_ke = key.second.steps[i - 1].kinetic_energy;
            auto const& curr_ke = key.second.steps[i].kinetic_energy;
            key.second.steps[i].energy_loss = prev_ke - curr_ke;
        }
