 ...
 * MuCF test geometry.
 *
 * Comprises a muon collimator, a d-t mixture target and neutron counters.
...
G4VPhysicalVolume* MucfTestGeo::build_geometry()
{
    ...
    double hydrogen_density = hydrogen_fraction * 1.007825031898 * g / mole
                              * liquid_hydrogen_density_ * target_.density;
    double deuteron_density = target_.deuteron_fraction * 2.01410178 * g / mole
                              * liquid_hydrogen_density_ * target_.density;
    double triton_density = target_.triton_fraction * 3.0160492 * g / mole
                            * liquid_hydrogen_density_ * target_.density;
    ...
    double env_size_xy = 40. * cm, env_size_z = 40. * cm;
    ...
    double detector_x = 16. * cm;
    double detector_y = 8. * cm;
    double detector_z = 2. * cm;
    double detector_t_yz_pos = 5.0 * cm;
    double rotation = 0. * deg;

    auto* scintillator_material
        = nist->FindOrBuildMaterial("G4_PLASTIC_SC_VINYLTOLUENE");
    auto* detector_t_solid = new G4Box(
        "det_t_solid", 0.5 * detector_x, 0.5 * detector_z, 0.5 * detector_y);
    ...
    for (int copy_num = 0; copy_num < 4; copy_num++)
    {
        ...
        auto detector_pos = G4ThreeVector(
            center_pos_x,
            detector_t_yz_pos * cos(rotation + (90.0 * deg) * copy_num),
            detector_t_yz_pos * sin(rotation + (90.0 * deg) * copy_num));
    ...
    auto* hole = new G4Tubs(
        "hole_solid", 0. * cm, 1.0 * cm, detector_z, 0. * deg, 360. * deg);
    auto* rotation_matrix_z = new G4RotationMatrix();
    rotation_matrix_z->rotateY(90. * deg);
    auto* detector_back_solid
        = new G4SubtractionSolid("det_b_solid",
    ...
    G4Box* solidDetF = new G4Box(
        "det_f_solid", 0.5 * detector_z, 0.5 * detector_y, 0.5 * detector_y);

    auto* detector_f_lv
        = new G4LogicalVolume(solidDetF, scintillator_material, "det_f_lv");
    new G4PVPlacement(nullptr,
                      G4ThreeVector(44.0 * cm, 0.0 * cm, 0.0 * cm),
                      detector_f_lv,
                      "det_f_pv",
                      world_lv,
                      false,
